...
sub login
{
  ...
  else
  {
   ...
   if($key == 4)
   {
    ...
    {
     my $sth = $dbh->prepare("SELECT room , check_in_date , check_out_date ,no_of_persons, no_of_days, amount FROM history WHERE name = ('$name')and phno = ('$phno')"); 
     $sth->execute();
     my @booked_rooms;
     my @booked_cid;
     my @booked_cod;
     ...
     { 
       ...
       BKR:my $newnum = <STDIN>;
        ...
        { 
         ...
         print"This room has been booked $flag times\n";
          my $sth = $dbh->prepare("SELECT booking_id FROM guest WHERE room = $newnum");
         $sth->execute();
         my @book_id;
       ...
       MOD:if($option == '1')
       {
      NCID:print"enter the new check in date :\n";
      my $newcid = <STDIN>;
     ...
     elsif($date1[1] == '02' && $date1[0]%4 == '0') 
     {
     ...
     elsif($date1[1] == '02' && $date1[0]%4 != '0') 
      {
        ...
        CH3:if($newcid =~ /^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/)
        {
         ...
         {
           print"invalid check in date to modify\n"; 
           goto NCID;
         }
     ...
     elsif($date2[1] == '02' && $date2[0]%4 == '0') 
     {
     ...
     elsif($date2[1] == '02' && $date2[0]%4 != '0') 
      {
        ...
        CH2:if($newcod =~ /^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/)
         ...
         {
           print"invalid check out date to modify\n"; 
           goto NCOD;
         }
        ...
        if($option == '3')
        {
         ...
         if($newnop =~ /^[1-3]$/)
         {
           my $sth = $dbh->prepare("update rooms set no_of_persons = ('$newnop') where room= $newnum and booking_id = ('$id')"); 
       $sth->execute();
    ...
    if($key == 6)
    {
     ...
     if($uname =~ $name && $pwd =~ $password)
     {
      ...
      {
       push(@booked_rooms,$booking_details[0]);
       push(@booked_cid,$booking_details[1]);
       push(@booked_cod,$booking_details[2]);
      }
       print"room no:\n";
       print"@booked_rooms\n";
       print"check in date:\n";
       ...
       my$len = scalar(@booked_rooms);
       print "enter the room no for cancellation : \n";
       BKROOM:my $booked_room = <STDIN>;
       my $flag = 0;
        for(my$i=0;$i<$len;$i++)
        {
       ...
       BKCID:print "enter the given check_in_date : \n";
       my $booked_cid = <STDIN>;
       my$len = scalar(@booked_cid);
      
       my $flag = 0;
       
        for(my$i=0;$i<$len;$i++)
        {
         if($booked_cid =~ $booked_cid[$i])
         {
          $flag = $flag+1;
         }
        }
        if($flag>=1)
        {
         print"selected room : $booked_cid\n";
         goto BKCOD;
        }
        else
        {
       ...
       BKCOD:print"enter the given check_out_date\n";
       my $booked_cod = <STDIN>;
       my$len = scalar(@booked_cod);
       my $flag = 0;
        for(my$i=0;$i<$len;$i++)
        {
         if($booked_cod =~ $booked_cod[$i])
         {
          $flag = $flag+1;
         }
        }
        if($flag>=1)
        {
         print"selected check out date : $booked_cod\n";
         goto CONFO;
        }
        else
        {
      ...
      if($conformation =~ 'ok')
      {
       ...
       $sth->execute();
       my $sth = $dbh->prepare("DELETE FROM guest where room = ('$booked_room') and phno = ('$reg_phno') and check_in_date = ('$booked_cid') and check_out_date = ('$booked_cod')"); 
       $sth->execute();
       my $sth = $dbh->prepare("DELETE FROM history where room = ('$booked_room') and phno = ('$reg_phno') and check_in_date = ('$booked_cid') and check_out_date = ('$booked_cod')"); 
       $sth->execute();
