 ...
 #selction options
  ...
  if($key == 1)
  {
   #getting check in date
    ...
    my $cod_return = $cod -> check_date();
    if($cod_return == 1)
    {
     goto COD;
    }
    my $compare = new Dateverification($check_in_date,$check_out_date); 
    $compare -> compare(); 
    ...
    if($uname =~ $usname && $pwd =~ $pass)
    {
     ...
     {    
       ...
       { 
        ...
        my $newcid = <STDIN>;
        my $ncid = new Dateverification($newcid);
        my $ncid_return = $ncid -> check_date();
        if($ncid_return == 1)
        { 
        goto MOD;
        } 
       ...
       if($option == '2')
       {
        ...
        my $newcod = <STDIN>;
        my $ncod = new Dateverification($newcod);
        my $ncod_return = $ncod -> check_date();
        if($ncod_return == 1)
        { 
        goto MOD;
        } 
