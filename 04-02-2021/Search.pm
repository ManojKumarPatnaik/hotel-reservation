  ...
  if($select =~ /^[1-4]$/)
    {
     if($select == 1)
     {
       my $sth = $dbh->prepare("SELECT count(room) FROM guest WHERE name = ('$name')and phno = ('$phno')");
       $sth->execute();
     ...
     if($select == 2)
      {
       my $sth = $dbh->prepare("SELECT sum(amount) FROM guest WHERE name = ('$name')and phno = ('$phno')");
       $sth->execute();
